...
?>
		<div class="container_12">
			...
			$result = mysqli_query($connection, "SELECT * FROM candidates_tbl, electrates_tbl, position_tbl
							...
							//student_id position_id image_link
                                ...
                                <tr>
									...
									$countvotes = mysqli_query($connection, "select polls from candidates_tbl WHERE position_id='1'");
									while ($row = mysqli_fetch_assoc($countvotes))
									{
										$p_totalvotes += $row["polls"];
									}

									$get_positions = mysqli_query($connection, "SELECT * FROM candidates_tbl
														  		  WHERE position_id = '1'");
									while($r=mysqli_fetch_array($get_positions))
									{
	...
	$poll = mysqli_fetch_array(mysqli_query($connection, "select * from position_tbl WHERE name='President'"));
	$position = $poll['name'];
	...
	$get_positions = mysqli_query($connection, "select * from candidates_tbl WHERE name='President'");
