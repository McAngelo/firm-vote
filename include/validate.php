		 ...
		 switch ($_REQUEST['action'])
		 {
			  /*-------------  This is the System Admin login code --------------*/
			   ...
			   if (isset($_POST['userlogin']) and isset($_POST['password']))
			   {
				...
				$result = mysqli_query($connection, $sql)
				 or die('Could not look up admin information; ' . mysqli_error());
				...
				if ($row = mysqli_fetch_array($result))
				{
					...
					$result2 = mysqli_query($connection, $sql_update) or die('Could not log in this user; ' . mysqli_error());
					 ...
					 if($row['acc_lvl'] == 1)
					 {
						 ...
						 redirect('../admin/system_admin/dashboard.php');
					 }
			...
			if (isset($_POST['lvl']))
		   {
				...
				$sql = "INSERT INTO level_tbl (level, date) " .
								"VALUES ('" . $lvl . "','" . $date . "') ";
				mysqli_query($connection, $sql) or die('Sorry Level registration failed; ' . mysqli_error());
			...
			if (isset($_POST['lvl']))
		   {
				...
				$sql = "UPDATE level_tbl SET level= '$lvl2', up_date= '$date1' WHERE ID= '$lvl_id'";
					...
					mysqli_query($connection, $sql) or die('Could not update level record; ' . mysqli_error());
			...
			if (isset($_POST['dep']))
		   {
				...
				$sql = "INSERT INTO department_tbl (dep_name, date_cr) " .
								"VALUES ('" . $dep . "','" . $date . "') ";
				mysqli_query($connection, $sql) or die('Sorry Department registration failed; ' . mysqli_error());
		  ...
		  /*------- This is the department update script---------*/
			...
			if (isset($_POST['dep']))
		   {
				...
				$sql = "UPDATE department_tbl SET dep_name= '$dep', up_date= '$date' WHERE ID= '$dep_id'";
					...
					mysqli_query($connection, $sql) or die('Could not update department record; ' . mysqli_error());
			...
			if (isset($_POST['position']))
		   {
				...
				mysqli_query($connection, $sql) or die('Sorry Position registration failed; ' . mysqli_error());
		  ...
		  /*------- This is the position update script---------*/
			...
			if (isset($_POST['position']))
		   {
				...
				$sql = "UPDATE position_tbl SET name= '$name', post_name= '$post', page_number= '$place', up_date= '$date' WHERE ID= '$post_id'";
					...
					mysqli_query($connection, $sql) or die('Could not update position record; ' . mysqli_error());
